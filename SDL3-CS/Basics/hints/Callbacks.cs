...
public static partial class SDL
{
    ...
    /// <para>This is called an initial time during <see cref="AddHintCallback(string,HintCallback,nint)"/> with the hint's
    /// current value, and then again each time the hint's value changes.</para>
    /// </summary>
    /// <param name="userdata">what was passed as <c>userdata</c> to <see cref="SDL.AddHintCallback(string,HintCallback,nint)"/>.</param>
    /// <param name="name">what was passed as <c>name</c> to <see cref="SDL.AddHintCallback(string,HintCallback,nint)"/>.</param>
