...
public static partial class SDL
{
    ...
    public enum InitFlags : uint
    {
        ...
        /// on Windows if you don't set <see cref="SDL.HintJoystickThread"/>
        /// </summary>
    ...
    /// </returns>
    /// <remarks>
    /// <para><see cref="Init"/> simply forwards to calling <see cref="InitSubSystem"/>.
    /// Therefore, the two may be used</para>
    /// <para>The file I/O (for example: <see cref="IOFromFile"/>) and threading (<see cref="CreateThread"/>)
    /// subsystems are initialized by default. Message boxes (<see cref="ShowSimpleMessageBox"/>) also attempt to work
    /// without initializing the video subsystem, in hopes of being useful in showing an error dialog when
    /// <see cref="Init"/> fails. You must specifically initialize other subsystems if you use them in your
    /// application.</para>
    ...
    /// <item><see cref="InitFlags.Video"/>: video subsystem; automatically initializes the events subsystem</item>
    ...
    /// </list>
    /// <para>Subsystem initialization is ref-counted, you must call <see cref="QuitSubSystem"/> for each
    /// <see cref="InitSubSystem"/> to correctly shutdown a subsystem manually (or call <see cref="Quit"/> to
    /// force shutdown). If a subsystem is already loaded then this call will increase the ref-count and return.</para>
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// <para>You should call this function even if you have already shutdown each initialized subsystem with
    /// <see cref="QuitSubSystem"/>. It is safe to call this function even in the case of errors in
    /// initialization.</para>
    /// <para>You can use this function with atexit() to ensure that it is run when your application is shutdown,
    /// but it is not wise to do this from a library or other dynamically loaded code.</para>
    /// </remarks>
    /// <seealso cref="Init"/>
    /// <seealso cref="QuitSubSystem"/>
