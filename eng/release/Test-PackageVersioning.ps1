...
$nativePackageRows = @($manifest.components | ForEach-Object {
    $component = $_
    [pscustomobject]@{
        Id = $component.packageId
        Project = $component.packageProject
...
foreach ($package in @($managedPackageRows + $nativePackageRows)) {
    ...
    if ($packageId -ne $package.Id) {
        Add-VersioningError "$($package.Project) PackageId '$packageId' does not match manifest package id '$($package.Id)'."
    }
