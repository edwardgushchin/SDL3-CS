...
foreach ($rid in $Rids) {
    ...
    foreach ($component in $components) {
        ...
        $sourceProject = Resolve-ReleasePath (Get-ReleaseNativePackageProjectForRid -Manifest $manifest -Component $component -Rid $rid)
        $sourcePackageRoot = Split-Path -Parent $sourceProject
        $sourceTargets = Join-Path $sourcePackageRoot "buildTransitive\$packageId.targets"
        if (-not (Test-Path -LiteralPath $sourceTargets -PathType Leaf)) {
            throw "Missing source targets for $($component.id): $sourceTargets"
        }

        $tempComponentPackageRoot = Join-Path $packageRoot $packageId
        ...
        Copy-Item -LiteralPath $sourceTargets -Destination (Join-Path $tempTargetsRoot "$packageId.targets") -Force
    ...
    $imports = @($components | ForEach-Object {
        $packageId = Get-ReleaseNativePackageIdForRid -Manifest $manifest -Component $_ -Rid $rid
        "  <Import Project=`"packages\$packageId\buildTransitive\$packageId.targets`" />"
