...
foreach ($componentId in $Components) {
    ...
    foreach ($rid in $Rids) {
        ...
        $expectedPackageId = Get-ReleaseNativePackageIdForRid -Manifest $manifest -Component $componentInfo -Rid $rid
        if ($receipt.PackageId -ne $expectedPackageId) {
            $errors.Add("Receipt package id mismatch for $componentId/$rid`: $($receipt.PackageId), expected $expectedPackageId.")
            $status = 'failed'
        }
