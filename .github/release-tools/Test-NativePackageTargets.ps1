...
foreach ($package in $nativePackages) {
    ...
    $targetsRelativePath = "buildTransitive\$($package.Id).targets"
    ...
    try {
        ...
        $packedTargets = @($projectXml.SelectNodes('/Project/ItemGroup/None') | Where-Object {
            $_.Include -eq $targetsRelativePath -and
            $_.Pack -eq 'true' -and
            $_.PackagePath -eq 'buildTransitive\$(PackageId).targets'
        })
        if ($packedTargets.Count -ne 1) {
            throw "$($package.Project) must pack $targetsRelativePath to buildTransitive\`$(PackageId).targets exactly once."
        }
        ...
        if ($package.NativePackagePlatform -in @('iOS', 'tvOS')) {
            ...
            foreach ($linkerToken in (Get-ExpectedLinkerTokens -Component $package.VersionComponent)) {
                Assert-TextContains -Text $targetsText -Expected $linkerToken -Context $targetsPath
            }

            if ($package.VersionComponent -eq 'SDL_shadercross') {
